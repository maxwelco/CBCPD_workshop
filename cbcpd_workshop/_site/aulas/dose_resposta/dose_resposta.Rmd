...
Vamos importar alguns pacotes até o final dessa aula, porém vamos trabalhar na base tidyverse. O tidyverse é um pacote que contém vários pacotes que auxiliam na manipulação de dados e construção de gráficos. Um exemplo que vamos usar é o pipe (%>%) do pacote magrittr (ou pode-se usar o |> da base R), que faz parte do tidyverse. Assim como o pacote `dplyr` para manipulação de dados.
...
Vamos usar o pacote `RCurl` para baixar os dados via GitHub.
...
```{r}
broom.mixed::tidy(model) |> #tidy é uma função do pacote broom.mixed
  mutate_if(is_double, ~round(.,2)) |> # inseri essa função apenas para arredondar para duas casas decimais
  reactable::reactable() # reactable é um pacote (e função) para formatar tabelas
```
...
```{r fig.width=6}
plot(model)
# escreva no console ?plot() e veja a quantidade de funções que você pode adicionar no gráfico
```
...
Observe que os resultados mudam (e muito!) quando usamos `type = "absolute"`. Existem duas tipos aqui: `type = "absolute"` ou `type = "relative"`. Tipo relativo (relative) é o padrão usado, então quando usamos a função `ED` sem o atributo type, o resultado será relativo. O significado é relativo ao limite superior da curva. Por outro lado, quando especificamos `type = "absolute"`, o resultado será o valor absoluto especificado, como 50 ou 90% de controle (valores do eixo-y).
...
O pacote `drc` ainda não está totalmente alinahdo com o `ggplot`. Por isso temos que seguir os seguintes passos para depois fazer o gráfico.
...
## Construindo o gráfico no `ggplot`.
...
ggplot(dose, aes(x = rate, 
  ...
  geom_ribbon(data=nd, aes(x=rate, 
                         ...
                         color = NULL),
              alpha=0.2) +
  theme_bw() + # tema do gráfico (veja o pacote ggthemes para mais opções)
  coord_trans(x="log") +
